$(document).ready(function () {
  ...
  fetchDataIndonesia("http://dev.farizdotid.com/api/daerahindonesia/provinsi", "select-provinsi", "provinsi");

  // fetch data kota saat value dari inputan provinsi berubah
  $(document).on("change", "#select-provinsi", function () {
    let id = $(this).val();

    fetchDataIndonesia(`http://dev.farizdotid.com/api/daerahindonesia/kota?id_provinsi=${id}`, "select-kota", "kota_kabupaten");
  });
});
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.each(data[responseKey], function (key, value) {
        $('#' + targetInputId).append('<option value="' + value.id + '">' + value.nama + '</option>');
      });
    }
  });
}